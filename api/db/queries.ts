import { pool } from "./pool";

export const getAllUsernames = async () => {
  const { rows } = await pool.query("SELECT * FROM users");
  return rows;
};

export const getUserProjectsBulk = async (userid: string) => {
  const { rows } = await pool.query(
    'SELECT p.id, p.name, p.userid, p.priority, p.description, p.datecreated as "dateCreated", p.due, COUNT(t.id) as "totalTasks", COUNT(CASE WHEN t.complete = true THEN t.id END) as "completedTasks" FROM projects as "p" LEFT JOIN tasks as "t" ON p.id = t.projectid WHERE p.userid = $1 GROUP BY p.id ORDER BY p.id',
    [userid],
  );
  return rows;
};

export const getUserProject = async (projectid: string) => {
  const { rows } = await pool.query(
    'SELECT p.id, p.name, p.description, p.datecreated as "dateCreated", p.due, p.userid, COUNT(t.id) as "totalTasks", COUNT(CASE WHEN t.complete = true THEN t.id END) as "completedTasks" FROM projects as "p" LEFT JOIN tasks as "t" ON p.id = t.projectid WHERE p.id = $1 GROUP BY p.id',
    [projectid],
  );
  return rows;
};

export const getProjectTasks = async (projectid: string | number) => {
  const { rows } = await pool.query(
    "SELECT * FROM tasks WHERE projectid = $1 ORDER BY position",
    [projectid],
  );
  return rows;
};

export const getTask = async (taskId: string | number) => {
  const { rows } = await pool.query("SELECT * FROM tasks WHERE id = $1", [
    taskId,
  ]);
  return rows[0];
};

export const verifyProjectOwnership = async (
  projectid: string | number,
  userid: string,
) => {
  const { rows } = await pool.query(
    "SELECT userid FROM projects WHERE id = $1",
    [projectid],
  );

  return rows[0]?.userid === userid;
};

export const verifyTaskOwnership = async (
  taskId: string | number,
  userid: string,
) => {
  const { rows } = await pool.query("SELECT userid FROM tasks WHERE id = $1", [
    taskId,
  ]);

  return rows[0]?.userid === userid;
};

export const getAdditionPosition = async (projectid: string | number) => {
  const { rows } = await pool.query(
    "SELECT MAX(position) FROM tasks WHERE projectid = $1",
    [projectid],
  );

  return Number(rows[0].max ?? 0) + 1;
};

export const addNewTask = async (
  name: string,
  description: string,
  complete: boolean,
  projectid: string | number,
  userid: string,
  position: number,
) => {
  const { rows } = await pool.query(
    "INSERT INTO tasks (name, description, complete, projectid, userid, position) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
    [name, description, complete, projectid, userid, position],
  );
  return rows;
};

export const addNewProject = async (
  name: string,
  userid: string,
  priority: number,
  description: string,
  due: string,
) => {
  const { rows } = await pool.query(
    'INSERT INTO projects (name, userid, priority, description, due) VALUES ($1, $2, $3, $4, $5) RETURNING id, name, userid, priority, description, datecreated as "dateCreated", due',
    [name, userid, priority, description, due],
  );
  return rows;
};

export const updateTask = async (
  name: string,
  description: string,
  id: string | number,
) => {
  const { rows } = await pool.query(
    "UPDATE tasks SET name = $1, description = $2 WHERE id = $3",
    [name, description, id],
  );
  return rows;
};

export const repositionTask = async (position: number, id: string | number) => {
  const { rows } = await pool.query(
    "UPDATE tasks SET position = $1 WHERE id = $2",
    [position, id],
  );
  return rows;
};

export const toggleTaskComplete = async (id: string | number) => {
  const { rows } = await pool.query(
    "UPDATE tasks SET complete = NOT complete WHERE id = $1",
    [id],
  );
  return rows;
};

export const updateProject = async (
  name: string,
  priority: number,
  description: string,
  due: string,
  id: string | number,
) => {
  const { rows } = await pool.query(
    "UPDATE projects SET name = $1, priority = $2, description = $3, due = $4 WHERE id = $5",
    [name, priority, description, due, id],
  );
  return rows;
};

export const accountForRemovedTask = async (
  position: number,
  projectid: string | number,
) => {
  const { rows } = await pool.query(
    "UPDATE tasks SET position = position-1 WHERE position > $1 AND projectid = $2",
    [position, projectid],
  );
  return rows;
};

export const accountForReorderedTasks = async (
  oldPosition: number,
  newPosition: number,
  projectid: string | number,
) => {
  if (oldPosition === newPosition) {
    return;
  }

  const { rows } =
    oldPosition < newPosition
      ? await pool.query(
          "UPDATE tasks SET position = position-1 WHERE position <= $1 AND position > $2 AND projectid = $3",
          [newPosition, oldPosition, projectid],
        )
      : await pool.query(
          "UPDATE tasks SET position = position+1 WHERE position >= $1 AND position < $2 AND projectid = $3",
          [newPosition, oldPosition, projectid],
        );

  return rows;
};

export const deleteProject = async (id: string | number) => {
  const { rows } = await pool.query("DELETE FROM projects WHERE id = $1", [id]);
  return rows;
};

export const deleteTask = async (id: string | number) => {
  const { rows } = await pool.query("DELETE FROM tasks WHERE id = $1", [id]);
  return rows;
};
